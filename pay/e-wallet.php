...
require '../lib/session_user.php';
	...
	require '../lib/session_login.php';
		...
		if (!$post_provider || !$post_pembayaran || !$post_jumlah) {
	        ...
	        $insert = $conn->query("INSERT INTO deposit VALUES ('','E-$kode', '$sess_username', 'BANK', '".$data_metod['provider']."' ,'".$data_metod['nama']."', '-','".$data_metod['tujuan']."','$reg', '$amount', 'Pending', 'Website', '$date', '$time')");
	        if ($insert == TRUE) {
		...
		<div class="alert alert-warning"><i class="mdi mdi-information fa-fw"></i> Baca <b>INFORMASI</b> yang terletak dikanan <i>Pc,Tablet</i> / dibawah <i>Smartphone</i> form sebelum melakukan deposit saldo.
				...
				<div class="row">
                    <div class="col-md-7">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="m-t-0 text-uppercase text-center header-title"><i class="ti-wallet text-primary"></i> DEPOSIT E-Wallet</h4><hr>
										...
										<input type="hidden" name="csrf_token" value="<?php echo $config['csrf_token'] ?>">
											<div class="form-group">
												...
												<?php
												$cek_kategori = $conn->query("SELECT * FROM metode_depo WHERE provider IN ('OVO','DANA','GOPAY','LINKAJA','SHOPEE-PAY') ORDER BY nama ASC");
